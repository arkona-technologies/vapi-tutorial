...
function update_cur_workspace(new_ws: string | null) {
   ...
   console.assert(!!el);
   console.log(el);
   (el as Element).textContent = `Current workspace directory: ${new_ws ?? "none"
      }`;
}
...
const DEFAULT_TIMEOUT_MS = 4000; // might even take longer than this if we're rsyncing?
...
async function request_section(calling_element: HTMLElement) {
   ...
   const successful = await (async () => {
      ...
      const maybe_result = await request_with_timeout(requested_section, DEFAULT_TIMEOUT_MS);
